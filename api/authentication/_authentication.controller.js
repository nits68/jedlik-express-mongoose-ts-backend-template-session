...
const _UserWithThatEmailAlreadyExists_exception_1 = tslib_1.__importDefault(require("../exceptions/_UserWithThatEmailAlreadyExists.exception"));
const _WrongCredentials_exception_1 = tslib_1.__importDefault(require("../exceptions/_WrongCredentials.exception"));
const _validation_middleware_1 = tslib_1.__importDefault(require("../middleware/_validation.middleware"));
const _user_dto_1 = tslib_1.__importDefault(require("../user/_user.dto"));
const _user_model_1 = tslib_1.__importDefault(require("../user/_user.model"));
const _logIn_dto_1 = tslib_1.__importDefault(require("./_logIn.dto"));
const _token_model_1 = tslib_1.__importDefault(require("./_token.model"));
class AuthenticationController {
    path = "/auth";
    router = (0, express_1.Router)();
    user = _user_model_1.default;
    token = _token_model_1.default;
    constructor() {
    ...
    initializeRoutes() {
        ...
        this.router.post(`${this.path}/register`, (0, _validation_middleware_1.default)(_user_dto_1.default), this.registration);
        this.router.post(`${this.path}/login`, (0, _validation_middleware_1.default)(_logIn_dto_1.default), this.login);
            ...
            if (user) {
                ...
                if (isPasswordMatching) {
                    ...
                    if (!user.email_verified) {
                        next(new _Http_exception_1.default(401, "Your Email has not been verified. Please click on resend!"));
                    }
                    else {
                        req.session.regenerate(error => {
                            if (error) {
                                next(new _Http_exception_1.default(400, error.message)); // to do
                            }
                    ...
                    next(new _WrongCredentials_exception_1.default());
                }
            }
            else {
                next(new _WrongCredentials_exception_1.default());
    ...
    registration = async (req, res, next) => {
        try {
            ...
            if (await this.user.findOne({ email: userData.email })) {
                next(new _UserWithThatEmailAlreadyExists_exception_1.default(userData.email));
            }
            else {
                ...
                await this.token
                    ...
                    .catch(error => {
                    next(new _Http_exception_1.default(500, error.message));
                });
                ...
                await mail_1.default
                    .send(msg)
                    .then(response => {
                    next(new _Http_exception_1.default(response[0].statusCode, `A verification e-mail has been sent to ${user.email}, It will be expire after one day.`));
                })
                    .catch(error => {
                    console.error(error);
                    next(new _Http_exception_1.default(400, error));
                });
            }
        }
        catch (error) {
    ...
    confirmEmail = (req, res, next) => {
        try {
            this.token
                ...
                .then(token => {
                if (!token) {
                    next(new _Http_exception_1.default(401, "We were unable to find a user for this verification. Please SignUp!"));
                }
                else {
                    this.user.findOne({ _id: token._userId, email: req.params.email }).then(user => {
                        if (!user) {
                            ...
                            next(new _Http_exception_1.default(200, "User has been already verified. Please Login!"));
                        }
                        else {
                            ...
                            this.user
                                ...
                                .then(() => {
                                next(new _Http_exception_1.default(200, "Your account has been successfully verified"));
                            })
                                .catch(error => {
                                next(new _Http_exception_1.default(500, error.mesage));
                            });
                        }
                    });
                }
    ...
    resendLink = (req, res, next) => {
        try {
            this.user.findOne({ email: req.params.email }).then(user => {
                if (!user) {
                    next(new _Http_exception_1.default(400, "We were unable to find a user with that email. Make sure your Email is correct!"));
                }
                else if (user.email_verified) {
                    next(new _Http_exception_1.default(200, "This account has been already verified. Please log in."));
                }
                else {
                    // create token:
                    const token = crypto_1.default.randomBytes(16).toString("hex");
    ...
    autoLogin = async (req, res, next) => {
        if (req.session.id && req.session.isAutoLogin) {
            const user = await _user_model_1.default.findById(req.session.user_id);
            if (user) {
                req.session.isLoggedIn = true;
                res.send(user);
            }
            else {
                next(new _Http_exception_1.default(404, "Please log in!"));
            }
        }
        else {
            next(new _Http_exception_1.default(404, "Please log in!"));
...
//# sourceMappingURL=_authentication.controller.js.map